...
根拠は **`reports/retro/` に集約**し、agent.md / instructions.md からは**リンク参照のみ**とする。

**Step 1: reports/ に事例ファイルを作成**

```
reports/retro/YYYY-MM-DD-{事象キーワード}.md
例: reports/retro/2026-01-25-gh300-loop.md
```

**Step 2: 反映先ファイルにはリンクのみ記載**

```markdown
> **根拠**: [2026-01-25 GH-300 ループ事例](../reports/retro/2026-01-25-gh300-loop.md)
```

**ルール:**
| 状況 | アクション |
|------|-----------|
| 新規ルール追加 | reports/ に事例作成 + リンク参照 |
| 既存ルールに根拠リンクあり | **追加しない**（既存維持） |
| 同一事例を複数ルールで参照 | 同じ reports/ ファイルをリンク |

> **原則**: agent.md / instructions.md に根拠詳細を直接書かない。`reports/retro/` への参照のみ。

**Example:**

```
Learning: Break complex tasks into numbered steps
Evidence: reports/retro/2026-01-20-gh500-numbering.md
...
- [ ] **Evidence in reports/** → 根拠詳細は `reports/retro/` に配置、agent.md にはリンクのみ
...
- **If evidence in agent.md**: `reports/retro/` に移動してリンク参照に変更
...
1. **Learning**: Subagent calls should include explicit success criteria
   - Evidence: 2026-01-20 GH-500 runSubagent 曖昧結果
...
2. **Learning**: Always validate subagent output format before processing
   - Evidence: 2026-01-20 GH-500 JSON期待→prose返却
